...
void bkg_PDF_tot() {
    ...
    // Load histogram
    TFile *file = TFile::Open("bkg_tot.root");
    TH1D *h = (TH1D*)file->Get("h_tot");
    ...
    //h->SetTitle("Bkg fit"); // Titre et axes
    h->GetXaxis()->SetTitle("mass (GeV)");
    h->GetYaxis()->SetTitle("Event / (1GeV)");
    h->SetMarkerStyle(20); // Style des marqueurs
    h->Draw("P");
    ...
    c1->SaveAs("H_yy_JZ_fit_bkg_tot.pdf");
    ...
    Canvas -> SaveAs("H_yy_JZ_PDF_bkg_tot.pdf");
